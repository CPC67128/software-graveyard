...
public final class Flou extends StdFilter implements Filter, ActionListener, ChangeListener, ItemListener, WindowListener {
	...
	public Flou() {
		...
		jsLvl = new JSlider(JSlider.HORIZONTAL, 0, 100, level);
			...
			for (i = 0, k = 0; i < h; i++) {
				for (j = 0; j < w; j++, k++) {
					if (i > 0 && i < h - 1 && j > 0 && j < w - 1) {
						add = destin[i-1][j-1] * coef + destin[i-1][j] * coef + destin[i-1][j+1] * coef
								+ destin[i][j-1] * coef + destin[i][j] + destin[i][j+1] * coef
								+ destin[i+1][j-1] * coef + destin[i+1][j] * coef + destin[i+1][j+1] * coef;
						source[k] = (int)(add / (1 + 8 * coef));
					}
					else {
						source[k] = destin[i][j];
					}
