...
public final class Prewitt implements Filter {
		...
		for (i = 0, k = 0; i < h; i++) {
			for (j = 0; j < w; j++, k++) {
				if (i > 0 && i < h - 1 && j > 0 && j < w - 1) {
					...
					yi = -1 * tab[i-1][j-1] + -1 * tab[i-1][j] + -1 * tab[i-1][j+1] +
						tab[i+1][j-1] + tab[i+1][j] + tab[i+1][j+1];

					xi = -1 * tab[i-1][j-1] + -1 * tab[i][j-1] + -1 * tab[i+1][j-1] +
