...
public final class SeuillageIn extends StdFilter implements Filter, ActionListener, ChangeListener, ItemListener, WindowListener {
...
for (int i = smSRC.getNumBands() - 1; i >= 0; i--) {
		...
		for (k = 0; k < source.length; k++) {
			source[k] = (int) ((( (int)((source[k] * scale) / 255.0f + nBas) - nBas) * 255) / (float)scale);
		}
